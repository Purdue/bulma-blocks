...
const { PanelBody, PanelRow, SelectControl, TextareaControl, TextControl, Button } = wp.components
...
registerBlockType( 'bulma-blocks/image', {
  ...
  icon: <svg aria-hidden="true" focusable="false" data-prefix="far" data-icon="file-image" className="svg-inline--fa fa-file-image fa-w-12" role="img" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 384 512"><path fill="#00c4a7" d="M369.9 97.9L286 14C277 5 264.8-.1 252.1-.1H48C21.5 0 0 21.5 0 48v416c0 26.5 21.5 48 48 48h288c26.5 0 48-21.5 48-48V131.9c0-12.7-5.1-25-14.1-34zM332.1 128H256V51.9l76.1 76.1zM48 464V48h160v104c0 13.3 10.7 24 24 24h104v288H48zm32-48h224V288l-23.5-23.5c-4.7-4.7-12.3-4.7-17 0L176 352l-39.5-39.5c-4.7-4.7-12.3-4.7-17 0L80 352v64zm48-240c-26.5 0-48 21.5-48 48s21.5 48 48 48 48-21.5 48-48-21.5-48-48-48z"></path></svg>, // Block icon from Dashicons → https://developer.wordpress.org/resource/dashicons/.
      ...
      [
          ...
          <PanelBody>
            ...
            <PanelRow>
              <TextControl
                ...
                onChange={ ( captionCredit ) => props.setAttributes( { captionCredit } ) }
              />
            </PanelRow>
          </PanelBody>
        </InspectorControls>,
        ...
        <div className={ 'bulma-blocks-editor-image' }>
          <MediaUploadCheck>
            <MediaUpload
              onSelect={ ( img ) => {
                props.setAttributes( { imgUrl: img.sizes.full.url, imgW: img.width, imgH: img.height, altText: props.attributes.altText !== '' ? props.attributes.altText : img.alt } )
              } }
              render={ ( { open } ) => {
                return props.attributes.imgUrl !== '' ?
                  (
                    <div className={ 'bulma-blocks-editor-image__preview' }>
                      <figure className={ `image ${ props.attributes.size }` } >
                        <img alt={ props.attributes.altText } src={ props.attributes.imgUrl } />
                      </figure>
                      <Button className={ 'bulma-blocks-editor-site-hero__button' } onClick={ open }>
                        Select a New Image
                      </Button>
                    </div>
                  ) :
                  (
                    ...
                    Pick an image from the media library.
                      ...
                      <Button className={ 'bulma-blocks-editor-image__button' } onClick={ open }>
                  Open Media Library
                      </Button>
                    </div>
                  )
  ...
  /**
	 ...
	 */
  save: props => {
    const returned = props.attributes.size !== '' ? (
      <div >
        <figure className={ `image ${ props.attributes.size } ${ props.attributes.align === '' ? 'full' : '' }` } style={ props.attributes.caption === '' ? { marginBottom: '3rem !important' } : '' } >
          <img alt={ props.attributes.altText } src={ props.attributes.imgUrl } />
        </figure>
        { props.attributes.caption !== '' ? (
          <div className={ 'bulma-blocks-image__caption' } >
            <p>{ props.attributes.caption }{ props.attributes.captionCredit !== '' ? <span>{ props.attributes.captionCredit }</span> : '' }</p>
          </div>
        ) : '' }
      </div>
    ) : (
      <figure className={ `image ${ props.attributes.size } ${ props.attributes.align } ${ props.attributes.align === '' ? 'full' : '' }` } style={ props.attributes.align !== '' ? { marginBottom: '0.5rem !important' } : { marginBottom: '3rem !important' } } >
        <img alt={ props.attributes.altText } src={ props.attributes.imgUrl } style={ props.attributes.align !== '' ? { maxWidth: `${ props.attributes.imgW }px`, maxHeight: `${ props.attributes.imgH }px` } : '' } />
        { props.attributes.caption !== '' ? (
          <div className={ 'bulma-blocks-image__caption' } style={ props.attributes.align !== '' ? { maxWidth: `${ props.attributes.imgW }px` } : '' }>
            <p>{ props.attributes.caption }{ props.attributes.captionCredit !== '' ? <span>{ props.attributes.captionCredit }</span> : '' }</p>
          </div>
        ) : '' }
      </figure>
